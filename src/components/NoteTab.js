...
const imageReferenceIndexManager = new ImageReferenceIndexManager();

const NoteTab = ({autoSaveInterval = 12, onShowStatus,stats,characterSettings,taskFieldMappings,expFormulas,quickAddTaskHint,customDomain, settings, onUpdateSettings}) => {
  ...
  const handleFileSave2 = async (content, fileName) => {
    if (!currentFile) return;
  ...
  const handleFileSave = async (content, fileName) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // 更新图片引用索引
        imageReferenceIndexManager.updateFileIndex(currentFile.id, currentFile.name, content, fileModifiedTime);
    ...
    <div className="note-tab">
      <div className="note-content">
        <FileExplorer
          ...
          imageReferenceIndexManager={imageReferenceIndexManager}
        />
